...
class EffectContext:
    ...
    def __init__(self, session, player_id: str, source: BoardEntity,
        ...
        # The entity a Trainer was dropped onto (play_targets); consumed by
        # the first choose_play_target, so a second choice still asks.
    ...
    async def choose_play_target(
        ...
        """The declared play target, used once, when it is among
        `candidates`; otherwise the ordinary one-card pick."""
        if self.play_target is not None and not self._play_target_consumed:
            self._play_target_consumed = True
            hit = next((card for card in candidates if card is self.play_target), None)
            if hit is not None:
                return hit
