...
class CardDefinition:
    ...
    # A definition that exists only on the board (a combined LEGEND) and is
    # never published as a collectible card.
    runtime_only = False
...
# --- LEGEND (HGSS): two physical halves, one Pokemon in play ------------------
...
def _validate_legend_static_attributes(attributes):
    """The half references are stamped on the assembled entity, never on an
    archetype."""
...
class LegendPokemonDef(PokemonCardDef):
    """The Pokemon that stands on the board once both halves are played
    together. Runtime-only: it is a definition (abilities, attacks, HP,
    types, its Prize count), not a collectible card, so the loader never
    publishes it and no deck can hold it."""
    ...
    def __init__(self, *, prize_count: int = 1, **kwargs):
        ...
        _validate_legend_static_attributes(kwargs.get("attributes"))
...
class LegendHalfCardDef(CardDefinition):
    """One collectible half. `legend` is the scripts-relative path (no .py)
    of the LegendPokemonDef script it belongs to; the loader binds the two
    halves to it after every script has loaded. In hand a half is a
    Pokemon card for searches; it never stands on the board by itself."""

    def __init__(self, *, legend: str, half: LegendHalf,
                 hp: Optional[int] = None, elements: Optional[List[PokemonTypes]] = None,
                 **kwargs):
        _validate_legend_static_attributes(kwargs.get("attributes"))
        ...
        self.legend = legend
        self.legend_definition: Optional[LegendPokemonDef] = None
        super().__init__(**kwargs)
        self.extra_attributes.update({
    ...
    def resolve_legend(self, definition):
        """Bind this printing to its combined definition."""
...
def matching_legend_halves(first, second) -> bool:
    """Whether two physical cards are opposite halves of one LEGEND."""
