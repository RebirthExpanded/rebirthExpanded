...
def main():
    ...
    for group in rules.EXCLUSIVE_NAME_GROUPS:
        ...
        if len(by_name) < 2:
            ...
            continue
        members = sorted(by_name.items())
        for name, guids in members:
        ...
        # Every pair in the group is a forbidden mix, at any split.
        from itertools import combinations
        for (n_a, guids_a), (n_b, guids_b) in combinations(members, 2):
            for n_a_count, n_b_count in ((1, 1), (2, 2), (4, 4)):
                ...
                check(not row["valid"] and "MustNotContain" in failure_types(row),
                      f"{label}: {n_a_count} {n_a} + {n_b_count} {n_b} is illegal")
            ...
            check(all(g.lower() in detail["offendingArchetypeIDs"]
                  ...
                  f"{label}: both offenders listed ({n_a} / {n_b})")
